...
class ServerThread(QThread):
    ...
    def process_sensor_data(self, sensor_data):
        ...
        # 时间同步
            ...
            if sensor_data['machine'] == 0:
                ...
                while True:
                    self.dialog_server(1211,sensor_data_now)
                    if sensor_data_now <= 5.0 and sensor_data['machine'] == 0:
                        self.dialog_server(123)
                        # print('甲烷浓度在预警值以下！')
                        self.send_command_to_client('methane_safe', True)
                        break
                    time.sleep(2)
                    sensor_data_now = round(sensor_data_now - 0.2, 2)
    ...
    def dialog_server(self, message_type, A=None, B=None):
        if message_type == 'start_server_success':
            ...
            message = '【警告】设备已断断电！'
            self.server_thread_signal.emit(message)
        elif message_type == 1211:
            message = f'【警告】当前甲烷浓度：{A}'
            self.server_thread_signal.emit(message)
